{
  description = "kanata-layer-indicator: persistent on-screen indicator of the active kanata layer";

  # No nix-wrapper-modules here: this is a quickshell overlay plus two watcher
  # scripts wired into a systemd user service, not a program with a CLI
  # worth wrapping.
  outputs = _: {
    homeModules.default =
      {
      ...
      let
        ...
        # Path of the symlink kanata's systemd service (the NixOS
        # services.kanata module) creates via --symlink-path, pointing at
        # its virtual output uinput device. RUNTIME_DIRECTORY there is
        # "kanata-${keyboard name}" and the symlink itself is named after
        # the keyboard, so for services.kanata.keyboards.main this is
        # always /run/kanata-main/main regardless of which /dev/input/eventN
        # the kernel assigns.
        kanataDevice = "/run/kanata-main/main";

        # Build the exact same quickshell derivation caelestia-shell already
        # builds for itself (nix/default.nix: `quickshell.withModules
        # [qt6.qtimageformats m3shapes]`), so this reuses that cached build
        # instead of compiling a second, differently-configured quickshell
        # from source.
        ...
        # tap-hold mod keys in kanata.kbd emit real lctrl/lshift/.../rctrl
        # key events, so this is the same signal the compositor itself acts
        # on. Requires the user to be in the "kanata-watch" group set up in
        # hosts/nixos/system.nix (read access to that one device, not all
        # input).
