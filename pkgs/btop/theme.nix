# colors -> btop's `themes.stylix` + `settings.color_theme`, ported from
# stylix's own btop target (modules/btop/hm.nix), since wrapping btop via
# nix-wrapper-modules bypasses `programs.btop` and so never runs Stylix's
# target for it.
#
# `colors`: a base16 palette as { base00 = "#hex"; ...; base0F = "#hex"; }
# (e.g. `config.lib.stylix.colors.withHashtag`), or null to leave btop's
