{
  ...
  # No nix-wrapper-modules here: home-manager's own programs.firefox
  # module already handles profile/prefs/extension management (real
  # typed options, not a flat config file a binary wrapper could bake
  # in), so this just relocates that same config unchanged.
  outputs = _: {
    homeModules.default =
      { config, pkgs, ... }:
      let
